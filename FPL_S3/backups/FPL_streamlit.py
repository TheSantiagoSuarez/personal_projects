...
def print_badge(team):
    akoya = pd.read_csv('FPL_S2/clean_csv/akoya.csv')
...
def display_stats(df, column, titles, metric):
    df = df[["manager_name",column]]
    df = df.sort_values(column,ascending=False).reset_index(drop=True,)
    
    win_manager = df.loc[0, "manager_name"]
    loss_manager = df.loc[len(df)-1, "manager_name"]

    st.subheader(titles)
    tab1, tab2, tab3 = st.tabs(["Most","Least","Table"])
    with tab1:
        if df.loc[0, column]==df.loc[1, column]:
            tied_manager= df.loc[1, "manager_name"]
            cols1,cols2 = st.columns(2)
            with cols1:
                st.image("FPL_S2/pictures/{}.png".format(win_manager.lower()),width=80)
            with cols2:
                st.image("FPL_S2/pictures/{}.png".format(tied_manager.lower()),width=80)
            st.text("{} & {}:".format(win_manager,tied_manager))        
            st.text("{} {}".format(round(df.loc[0, column]),metric))
        else:
            st.image("FPL_S2/pictures/{}.png".format(win_manager.lower()),width=160)
            st.text("{}:".format(win_manager))        
            st.text("{} {}".format(round(df.loc[0, column]),metric))

    with tab2:
        if df.loc[len(df)-1, column]==df.loc[len(df)-2, column]:
            tied_manager= df.loc[len(df)-2, "manager_name"]
            cols1,cols2 = st.columns(2)
            with cols1:
                st.image("FPL_S2/pictures/{}.png".format(loss_manager.lower()),width=80)
            with cols2:
                st.image("FPL_S2/pictures/{}.png".format(tied_manager.lower()),width=80)
            st.text("{} & {}:".format(loss_manager,tied_manager)) 
            st.text("{} {}".format(round(df.loc[len(df)-1, column]),metric))
        else: 
            st.image("FPL_S2/pictures/{}.png".format(loss_manager.lower()),width=160)
...
def points():
    ...
    data = pd.read_csv('FPL_S2/findings/points/gk_ranking.csv')
    data_gw = pd.read_csv('FPL_S2/findings/points/gw_gk_ranking.csv')
    ...
    data = pd.read_csv('FPL_S2/findings/points/def_ranking.csv')
    data_gw = pd.read_csv('FPL_S2/findings/points/gw_def_ranking.csv')
    ...
    data = pd.read_csv('FPL_S2/findings/points/mid_ranking.csv')
    data_gw = pd.read_csv('FPL_S2/findings/points/gw_mid_ranking.csv')
    ...
    data = pd.read_csv('FPL_S2/findings/points/fwd_ranking.csv')
    data_gw = pd.read_csv('FPL_S2/findings/points/gw_fwd_ranking.csv')
    ...
    data = pd.read_csv('FPL_S2/findings/points/bench.csv')
    ...
    data = pd.read_csv('FPL_S2/findings/points/bench_best.csv')
    ...
    data = pd.read_csv('FPL_S2/findings/points/podiums.csv')
    ...
    data = pd.read_csv('FPL_S2/findings/points/tottenham.csv')
    ...
    data = pd.read_csv('FPL_S2/findings/points/last_df.csv')
...
def players():
    ...
    #region Unloyalty?
    manager_df = data[data["manager_name"]==manager].sort_values("player_id",ascending=True).reset_index(drop=True).iloc[:10]
    cols = st.columns(5)
    ...
    #region Most Played
    data = pd.read_csv('FPL_S2/findings/players/most_played.csv')
    manager_df_full = data[data["manager_name"]==manager].reset_index(drop=True).sort_values("player_id",ascending=False)
    ...
    #region Most Teams
    most_teams = pd.read_csv('FPL_S2/findings/players/most_teams.csv')[:8]
    ...
    #region Club Mascot
    mascot = pd.read_csv('FPL_S2/findings/players/club_mascot.csv')
    fielded = mascot[mascot["manager_name"]==manager].sort_values("fielded",ascending=False).reset_index(drop=True)
    ppg = mascot[(mascot["manager_name"]==manager)&(mascot["fielded"]>15)].sort_values("ppg",ascending=False).reset_index(drop=True)
    ...
    with tab1:
        ...
        for i in range(3):
            with tab1_cols[i]:
                row = fielded.loc[i]
                percentage = round(row["points"]/int(real_ranking[real_ranking["manager_name"]==manager]["points"].values[0]),2)
                print_badge(row["team"])
    ...
    #region Star Players
    data = pd.read_csv('FPL_S2/findings/players/most_played.csv')
    total_points = data[data["manager_name"]==manager].sort_values("points",ascending=False).reset_index(drop=True)[:3]
    ppg = data[(data["manager_name"]==manager)&(data["player_id"]>5)].sort_values("ppg",ascending=False).reset_index(drop=True)[:3]
...
def stats():
    data = pd.read_csv("FPL_S2/findings/stats/stats.csv")
...
def transfers():
    ...
    #region Transfer In
    data = pd.read_csv("FPL_S2/findings/transfers/most_in.csv")
    ...
    with tab1:
        if manager=="Everyone":
            ...
            manager_df = data[data["manager_name"]==manager].reset_index(drop=True)
            cols = st.columns(2)
            for i in range(2):
                with cols[i]:
    ...
    data = pd.read_csv("FPL_S2/findings/transfers/transfers_net_points.csv")
    ...
    with tab1:
        if manager=="Everyone":
            ...
            df = data[data["manager_name"]==manager].reset_index(drop=True).rename(columns={"net_points":"player_id"})
            cols = st.columns(5)
            for i in range(5):
                with cols[i]:
                    show_player(df,i,"manager","net points")
